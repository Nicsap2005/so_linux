...
class MainApp:
    ...
    def _performances_btn_function(self,performance_instance, window, performances_label, graph_label,text_box,tree_box,scrollbar):
        self._show_label()
        self._else_btn_function(performances_label, graph_label,text_box,tree_box,scrollbar)
        performance_instance.performances_btn(window, performances_label, graph_label)
        
    def _utility_btn_function(self,utility_instance, window, performances_label, graph_label,text_box,tree_box,scrollbar):
        self._show_label()
        self._else_btn_function(performances_label, graph_label, text_box,tree_box,scrollbar)
        utility_instance.util_btn(window, performances_label, graph_label, text_box,tree_box,scrollbar)
    
    def _else_btn_function(self,performances_label, graph_label,text_box,tree_box,scrollbar):
        self.performance_instance.elsebtn(performances_label, graph_label)
        self.utility_instance.elsebtn(performances_label, graph_label,text_box,tree_box,scrollbar)
    ...
    def _setup_window(self):
        ...
        performances_btn = tk.Button(self.window, width=12, height=2, text="Performances", borderwidth=2, relief="solid", 
                                     command=lambda: self._performances_btn_function(self.performance_instance, self.window, self.performances_label, self.graph_label,self.text_box,self.tree_box,self.scrollbar))
        ...
        utility_btn = tk.Button(self.window, width=12, height=2, text="Utility", borderwidth=2, relief="solid", 
                                command=lambda:self._utility_btn_function(self.utility_instance, self.window, self.performances_label, self.graph_label,self.text_box,self.tree_box,self.scrollbar))
        utility_btn.grid(row=2, column=0)
        
        # Another button
        else_btn = tk.Button(self.window, width=12, height=2, text="Else", borderwidth=2, relief="solid", command=lambda:self._else_btn_function(self.performances_label, self.graph_label,self.text_box,self.tree_box,self.scrollbar))
